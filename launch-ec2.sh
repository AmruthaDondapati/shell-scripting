...
SG_ID=$(aws ec2 describe-security-groups --filters Name=group-name,Values=My_Batch_52 --region us-east-1 | jq .SecurityGroups[].GroupId | sed -e 's/"//g')
...
launch_ec2() { 
    ...
    aws ec2 create-default-subnet --availability-zone us-east-1a &>> "${LOFGILE}"
